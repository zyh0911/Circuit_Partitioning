                                                                            ...
                                                                            bool MHEC)
{
    ...
    for (auto e_it = sortedEdge.begin(); e_it != sortedEdge.end(); e_it++)
    {
        ...
        if ((!restricted || (restricted && preservePartition(inst, nodesConnected, count))) && nodesNotMatched(nodesConnected, count, nodesRemain) && graph->sizeUnderLimit(nodesConnected, count, part, spaceLimit[part]))
        {
    ...
    if (MHEC)
    {
        for (auto e_it = sortedEdge.begin(); e_it != sortedEdge.end(); e_it++)
        {
            ...
            if (!unmatchedNodes0->empty() && size0 <= spaceLimit[0])
            {
                ...
                for (auto it = unmatchedNodes0->begin(); it != unmatchedNodes0->end(); it++)
                    nodesRemain[*it] = false;
            }
            else
            ...
            if (!unmatchedNodes1->empty() && size1 <= spaceLimit[1])
            {
                ...
                for (auto it = unmatchedNodes1->begin(); it != unmatchedNodes1->end(); it++)
                    nodesRemain[*it] = false;
            }
            else
